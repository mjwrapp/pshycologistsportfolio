...
import { bookAppointment, verifyPayment } from '../services/api';
...
export default function BookingPage() {
  ...
  useEffect(() => {
    const basePrice = selectedMode === 'video' ? 500 : 300;
    setAmount(selectedDuration === '60min' ? basePrice * 2 : basePrice);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle the case where the API response might structure bookingId differently
      // e.g. bookingData.booking._id OR bookingData.bookingId
      const bookingId = bookingData.booking?._id || bookingData.bookingId;
      if (!bookingId) throw new Error("Did not receive a valid booking ID from server.");
      ...
      setTimeout(async () => {
        try {
          ...
          console.log('Payment verified completely:', verifyResponse);

          // 5. Show Success states and clear form
          setSuccess(true);
          setName('');
          setPhone('');
          setEmail('');
          ...
          console.error('Payment verification failed on backend:', error);
          setErrorMsg(error.message || 'Payment was successful but verification failed.');
        } finally {
          setIsLoading(false);
        }
      }, 1500);
      ...
      console.error('Frontend API/Flow Error:', err);
      setErrorMsg(err.message || 'Failed to connect to the server or process payment.');
    } finally {
      // Don't set loading false immediately if successful, because modal handles resting state
      setIsLoading(false);
    }
  };
        ...
        <div className="glass p-8 md:p-12 rounded-[3.5rem] shadow-[0_20px_50px_rgba(154,130,185,0.15)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)]">
          ...
          {success && (
            <div className="mb-6 p-4 bg-green-500/10 border border-green-500/20 text-green-600 rounded-xl">
              Booking successfully initiated! Waiting for payment.
            </div>
          )}
            ...
            <div className="lg:col-span-3 space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-white/60 dark:bg-black/30 border border-black/10 dark:border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-primary focus:ring-4 ring-primary/10 transition-all" placeholder="Jane Doe" required />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2 opacity-80 pl-2">Phone</label>
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full bg-white/60 dark:bg-black/30 border border-black/10 dark:border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-primary focus:ring-4 ring-primary/10 transition-all" placeholder="+1..." required />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium mb-2 opacity-80 pl-2">Email Address</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-white/60 dark:bg-black/30 border border-black/10 dark:border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-primary focus:ring-4 ring-primary/10 transition-all" placeholder="jane@example.com" required />
              </div>
              ...
              <div className="pt-4">
                ...
                <textarea rows={3} value={message} onChange={(e) => setMessage(e.target.value)} className="w-full bg-white/60 dark:bg-black/30 border border-black/10 dark:border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-primary focus:ring-4 ring-primary/10 transition-all resize-none" placeholder="What's on your mind?"></textarea>
              </div>
            </div>
              ...
              <div className="bg-white/50 dark:bg-black/20 rounded-[2.5rem] p-8 border border-black/5 dark:border-white/5 h-full flex flex-col justify-between shadow-inner relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="bg-primary/10 dark:bg-primary/20 p-5 rounded-2xl space-y-3 mb-6">
                    <div className="flex justify-between font-medium">
                      <span>{selectedDuration} {selectedMode} Session</span>
                      <span>${amount}</span>
                    </div>
                ...
                <div className="relative z-10">
                  <motion.button
                    ...
                    disabled={isLoading}
                    type="submit"
                    className="w-full bg-primary hover:bg-primary-dark text-white font-bold py-5 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-xl shadow-primary/30 tracking-wide text-lg disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    <Lock size={18} /> {isLoading ? 'Processing...' : 'Proceed to Payment'}
                  </motion.button>
